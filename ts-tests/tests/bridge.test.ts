...
describeCrossChainTransfer('Test Cross-chain Transfer', ``, (context) => {
    ...
    step("Boundary testing on ethereum: over the maximum balance", async function () {
        ...
        await signAndSend(context.parachainConfig.api.tx.bridgeTransfer.transferNative(
                handlerBalance.div(BigNumber.from(1000000)).add(BigNumber.from(100)).add(BigNumber.from(fee.toString())),
                receipt,
                0
            ),
                ...
                allRecords.filter(({phase}) =>
                    ...
                    if (context.parachainConfig.api.events.system.ExtrinsicFailed.is(event)) {
                        ...
                        if (dispatchError.isModule) {
                            ...
                            const decoded = context.parachainConfig.api.registry.findMetaError(dispatchError.asModule);
