...
async function deployBridgeContracts(wallet: Wallet) {
    ...
    const ERC20HandlerFactory = new ethers.ContractFactory(ERC20HandlerContract.abi, ERC20HandlerContract.bytecode, wallet);
    const ERC721HandlerFactory = new ethers.ContractFactory(ERC721HandlerContract.abi, ERC721HandlerContract.bytecode, wallet);
    const GenericHandlerFactory = new ethers.ContractFactory(GenericHandlerContract.abi, GenericHandlerContract.bytecode, wallet);
    ...
    const erc20Handler = await ERC20HandlerFactory.deploy(bridge.address, initialResourceIDs, initialContractAddresses, burnableContractAddresses);
    const erc721Handler = await ERC721HandlerFactory.deploy(bridge.address, initialResourceIDs, initialContractAddresses, burnableContractAddresses);
    const genericHandler = await GenericHandlerFactory.deploy(bridge.address, initialResourceIDs, initialContractAddresses, initialDepositFunctionSignatures, initialExecuteFunctionSignatures);
...
async function setupCrossChainTransfer(pConfig: ParachainConfig, eConfig: EthConfig, ethRelayers: [string], parachainRelayers: [string]) {
    ...
    if (resource.toHuman() !== "BridgeTransfer.transfer") {
        extrinsic.push(pConfig.api.tx.sudo.sudo(pConfig.api.tx.chainBridge.setResource(destResourceId, "BridgeTransfer.transfer")))
    }
...
function generateBridgeConfig(eConfig: EthConfig, ethRelayer: string, parachainRelayer: string, ethStartFrom: number, parachainStartFrom: number, filename: string) {
...
async function startChainBridge(ethConfig: EthConfig, parachainConfig: ParachainConfig, ethRelayer: string, parachainRelayer: string, config: string, log: string) {
    ...
    const lsProcess = spawn(
        '/root/dev-spaces/litentry/ChainBridge/build/chainbridge',
    ...
    describe(title, function () {
        ...
        before('Deploying Bridge Contracts', async function () {
            ...
            const {
                ...
                genericHandler,
                erc20
            } = await deployBridgeContracts(wallets.alice);
