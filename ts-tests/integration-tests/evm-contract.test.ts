...
import { signAndSend, describeLitentry, loadConfig, sleep, sudoWrapper} from '../common/utils';
...
describeLitentry('Test EVM Module Contract', ``, (context) => {
    ...
    step('Transfer Value from Eve to EVM external account', async function () {
        ...
        if ('Test' !== filterMode) {
            let extrinsic = sudoWrapper(context.api, context.api.tx.sudo.sudo(context.api.tx.extrinsicFilter.setMode('Test')));
        ...
        // In case evm is not enabled in Normal Mode, switch back to filterMode, after test.
        let extrinsic = sudoWrapper(context.api, context.api.tx.sudo.sudo(context.api.tx.extrinsicFilter.setMode(filterMode)));
        await signAndSend(extrinsic, context.alice);
    });
    ...
    step('Deploy and test contract by EVM external account', async function () {
        ...
        if ('Normal' !== filterMode) {
            let extrinsic = sudoWrapper(context.api, context.api.tx.sudo.sudo(context.api.tx.extrinsicFilter.setMode('Normal')));
            await signAndSend(extrinsic, context.alice);
        }
