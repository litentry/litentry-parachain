...
describe('Raw', () => {
  let u8a;
  beforeEach(() => {
    u8a = new _.Raw(registry, [1, 2, 3, 4, 5]);
  });
  ...
  it('allows wrapping of a pre-existing instance', () => {
    expect(new _.Raw(registry, u8a).length).toEqual(5);
  });
  ...
  describe('utils', () => {
    ...
    it('has valid isAscii', () => {
      ...
      expect(new _.Raw(registry, '0x2021222324').isAscii).toBe(true);
    });
    it('has valid toUtf8', () => {
      expect(new _.Raw(registry, 'Приветствую, ми').toUtf8()).toEqual('Приветствую, ми');
      expect(new _.Raw(registry, '0xe4bda0e5a5bd').toUtf8()).toEqual('你好');
    });
    it('throws on invalid utf8', () => {
      expect(() => new _.Raw(registry, '0x7f07b1f87709608bee603bbc79a0dfc29cd315c1351a83aa31adf7458d7d3003').toUtf8()).toThrow(/The character sequence is not a valid Utf8 string/);
    });
  });
});