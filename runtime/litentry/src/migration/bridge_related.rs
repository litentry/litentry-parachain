...
use pallet_bridge::{
	BridgeFee, Resources, ResourceId, BridgeChainId,
};
use pallet_assets_handler::{
	AssetInfo, ResourceToAssetInfo, ExternalBalances, MaximumIssuance,
};
...
pub const native_token_resource_id: [u8; 32] = hex!("0000000000000000000000000000000a21dfe87028f214dd976be8479f5af001");
		...
		+ pallet_balances::Config<Balance = u128>,
{
	pub fn relocate_resource_fee_storage() -> frame_support::weights::Weight {
		...
		let stored_data_resources: Vec<_> = storage_key_iter::<
			ResourceId,
			Vec<u8>,
			Blake2_256,
		>(pallet_prefix, storage_item_prefix_resources)
		.collect();
		...
		let stored_data_fee: Vec<_> = storage_key_iter::<
			BridgeChainId,
			u128,
			Twox64Concat,
		>(pallet_prefix, storage_item_prefix_fee)
		.collect();
		...
		migrated_count_resources.saturating_add(migrated_count_fee).saturating_mul(weight.write + weight.read)
	}
	pub fn delete_bridge_balances_storage() -> frame_support::weights::Weight {
		...
		// Assert that old storage is empty
		assert!(<BridgeBalances<T>>::iter()
		.next()
		.is_none());

		let weight = T::DbWeight::get();
		frame_support::weights::Weight::from_parts(0, result.unique.into().saturating_mul(weight.write + weight.read))
	}
		...
		+ pallet_balances::Config<Balance = u128>,
{
	pub fn pre_upgrade_resource_fee_storage() -> Result<Vec<u8>, &'static str> {
		...
		assert_eq!(resources_iter.next(), Some((native_token_resource_id, b"BridgeTransfer.transfer".to_vec())));
	...
	pub fn post_upgrade_resource_fee_storage(_state: Vec<u8>) -> Result<(), &'static str> {
		...
		assert_eq!(new_resource_fee_iter.next(), Some((native_token_resource_id, expected_asset_info)));
		Ok(())
	}
	...
	pub fn pre_upgrade_external_balance_storage() -> Result<Vec<u8>, &'static str> {
		...
		let storage_item_prefix: &[u8] = b"ExternalBalances";
		let stored_data =
			get_storage_value::<u128>(pallet_prefix, storage_item_prefix, b"")
				.expect("Storage query fails: BridgeTransfer ExternalBalances");
		...
		+ pallet_balances::Config<Balance = u128>,
{
	...
	fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
		...
		Ok((
			external_balances_vec,
			maximum_issuance_vec,
		)
			.encode())
	}
