...
OUTPUT=$(${CLIENT} trusted -d link-identity //Bob did:litentry:substrate:${FIRST_NEW_ACCOUNT} litentry) || { echo "Link identity command failed"; exit 1; }
...
OUTPUT=$(${CLIENT} trusted -d link-identity //Bob "did:litentry:substrate:${SECOND_NEW_ACCOUNT}" litentry -d //Eve) || { echo "Link identity command failed"; exit 1; }
echo "Finished Linking identity to Bob"
sleep 30
