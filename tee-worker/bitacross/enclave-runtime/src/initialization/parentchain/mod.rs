	...
	match ParentchainInitParams::decode(&mut encoded_params.as_slice())? {
		ParentchainInitParams::Parachain { id, shard, params } => {
			...
			match id {
				ParentchainId::Litentry => {
					let handler =
						IntegriteeParachainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
				...
				ParentchainId::TargetA => {
					let handler =
						TargetAParachainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
						.execute_on_validator(|v| v.latest_finalized_header())?;
				...
				ParentchainId::TargetB => {
					let handler =
						TargetBParachainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
						.execute_on_validator(|v| v.latest_finalized_header())?;
		...
		ParentchainInitParams::Solochain { id, shard, params } => {
			...
			match id {
				ParentchainId::Litentry => {
					let handler =
						IntegriteeSolochainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
						.execute_on_validator(|v| v.latest_finalized_header())?;
				...
				ParentchainId::TargetA => {
					let handler =
						TargetASolochainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
						.execute_on_validator(|v| v.latest_finalized_header())?;
				...
				ParentchainId::TargetB => {
					let handler =
						TargetBSolochainHandler::init(base_path, params, shard_creation_info)?;
					let header = handler
						.validator_accessor
						.execute_on_validator(|v| v.latest_finalized_header())?;
