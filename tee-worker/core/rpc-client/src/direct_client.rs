...
use itp_utils::{info as lit_info, FromHexPrefixed, ToHexPrefixed};
use log::*;
...
impl DirectApi for DirectClient {
	fn get(&self, request: &str) -> Result<String> {
		...
		lit_info!("[WorkerApi Direct]: (get) Sending request: {:?}", request);
	...
	fn watch(&self, request: String, sender: MpscSender<String>) -> JoinHandle<()> {
		lit_info!("[WorkerApi Direct]: (watch) Sending request: {:?}", request);
