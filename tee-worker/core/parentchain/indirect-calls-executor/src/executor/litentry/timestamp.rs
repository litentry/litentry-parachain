...
use itp_types::{extrinsics::ParentchainUncheckedExtrinsicWithStatus, TimestampCallFn};
...
pub fn decode(encoded_xt_opaque: &mut &[u8]) -> Result<u64> {
	...
	let call: Result<ParentchainUncheckedExtrinsicWithStatus<([u8; 2], Compact<u64>)>> =
		ParentchainUncheckedExtrinsicWithStatus::<TimestampCallFn>::decode(encoded_xt_opaque)
			.map_err(|e| e.into());
