...
impl LinkIdentity {
	...
	{
		let (_, (shard, account, encrypted_identity, encrypted_validation_data, nonce)) =
			&extrinsic.function;
		...
		debug!(
			"execute indirect call: LinkIdentity, who: {:?}, identity: {:?}",
...
impl<R, S, T, N> Executor<R, S, T, N> for LinkIdentity
where
	R: AccessKey,
	R::KeyType: ShieldingCryptoDecrypt<Error = itp_sgx_crypto::Error>
		+ ShieldingCryptoEncrypt<Error = itp_sgx_crypto::Error>,
	...
	N::MetadataType: IMPCallIndexes + TeerexCallIndexes + VCMPCallIndexes + UtilityCallIndexes,
{
	...
	fn call_index_from_metadata(&self, metadata_type: &N::MetadataType) -> Result<[u8; 2]> {
		metadata_type.link_identity_call_indexes().map_err(|e| e.into())
	}
		...
		if self.execute_internal(context, &extrinsic).is_err() {
			...
			if let Err(internal_e) = context.submit_trusted_call_from_error(
				*shard,
				...
				warn!("fail to handle internal errors in link_identity: {:?}", internal_e);
			}
			return Err(e)
		}
		Ok(())
