...
pub fn enclave_signer_signatures_are_valid() {
	...
	let trusted_call = TrustedCall::noop(Identity::Substrate(enclave_account.into()));
...
pub fn nonce_is_computed_correctly() {
	...
	let trusted_call_1 = TrustedCall::noop(Identity::Substrate(enclave_account.clone().into()));
	...
	let trusted_call_2 = TrustedCall::noop(Identity::Substrate(enclave_account.clone().into()));
