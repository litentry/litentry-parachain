...
use itc_parentchain::indirect_calls_executor::{
	filter_calls::LitentryCallFilter, parentchain_extrinsic_parser::ParentchainExtrinsicParser,
...
pub fn submit_shielding_call_to_top_pool() {
	...
	let indirect_calls_executor =
		IndirectCallsExecutor::<_, _, _, _, LitentryCallFilter<ParentchainExtrinsicParser>>::new(
			shielding_key_repo,
			enclave_signer,
			top_pool_author.clone(),
