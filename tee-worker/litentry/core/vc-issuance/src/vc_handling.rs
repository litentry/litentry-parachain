...
use crate::vc_primitives::{VCRequest, VCResponse};
...
where
	...
	O: EnclaveOnChainOCallApi,
{
	pub fn start(&self, sender: std::sync::mpsc::Sender<(VCResponse, Sender<Vec<u8>>)>) {
	...
	fn on_process(&self) -> Result<(H256, H256, Vec<u8>), VCMPError> {
