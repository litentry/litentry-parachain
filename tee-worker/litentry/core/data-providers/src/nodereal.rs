...
impl Default for NoderealClient {
	fn default() -> Self {
		Self::new()
	}
}
...
impl NoderealClient {
	pub fn new() -> Self {
		let api_key = GLOBAL_DATA_PROVIDER_CONFIG.read().unwrap().nodereal_api_key.clone();
		let api_url = GLOBAL_DATA_PROVIDER_CONFIG.read().unwrap().nodereal_api_url.clone();
