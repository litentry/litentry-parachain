...
library MoralisClient {
    function getSolanaNativeBalance(
        ...
        url = string(abi.encodePacked(getNetworkUrl(network), "/", account, "/balance"));
    ...
    function getSolanaTokensBalance(
        ...
        url = string(abi.encodePacked(getNetworkUrl(network), "/", account, "/tokens"));
        HttpHeader[] memory headers = new HttpHeader[](1);
        headers[0] = HttpHeader("X-API-Key", apiKey);
    ...
    function getErcTokensBalance(
        ...
        url = string(
            abi.encodePacked(getNetworkUrl(network), "/", account, "/erc20", "?chain=", chain)
        );
        HttpHeader[] memory headers = new HttpHeader[](1);
        headers[0] = HttpHeader("X-API-Key", apiKey);
