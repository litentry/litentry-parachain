...
pub fn build(req: &AssertionBuildRequest, rtype: GenericDiscordRoleType) -> Result<Credential> {
	let role_id = get_generic_discord_role_id(&rtype);
...
fn get_generic_discord_role_id(rtype: &GenericDiscordRoleType) -> String {
	match rtype {
		GenericDiscordRoleType::Legend => GLOBAL_DATA_PROVIDER_CONFIG
			.read()
			.unwrap()
			.contest_legend_discord_role_id
			.clone(),
		GenericDiscordRoleType::Popularity => GLOBAL_DATA_PROVIDER_CONFIG
			.read()
			.unwrap()
			.contest_popularity_discord_role_id
			.clone(),
		GenericDiscordRoleType::Participant => GLOBAL_DATA_PROVIDER_CONFIG
			.read()
			.unwrap()
