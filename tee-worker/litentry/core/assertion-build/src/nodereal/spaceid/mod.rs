...
use lc_data_providers::{
	nodereal::{DomainInfo, NoderealClient, SpaceIDApiList},
	DataProviderConfigReader, ReadDataProviderConfig,
};
use serde::{Deserialize, Serialize};
use std::string::ToString;

pub struct SPACEIDLogic;
pub trait SPACEIDLogicInterface {
	fn get_bnb_domain_holding_amount(addresses: &[String]) -> Result<usize>;
}
...
impl SPACEIDLogicInterface for SPACEIDLogic {
	fn get_bnb_domain_holding_amount(owners: &[String]) -> Result<usize> {
		...
		let owned_domains: Domains = Domains::from_value(&response)
			.map_err(|_e| Error::UnclassifiedError(ErrorDetail::ParseError))?;
...
impl Domains {
	pub fn from_value(value: &serde_json::Value) -> Result<Domains> {
				...
				if let Some(values) = val.as_array() {
					for info in values {
						let domain_info: DomainInfo =
							DomainInfo::from_value(info).map_err(Error::UnclassifiedError)?;
