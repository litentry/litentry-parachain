...
const VC_DEV_UNDERGRADUATE_SUBJECT_DESCRIPTION: &str =
	"You are an outstanding student of the OneBlock+ Substrate developer course.";
const VC_DEV_UNDERGRADUATE_SUBJECT_TYPE: &str = "OneBlock+ Outstanding Substrate Developer";

pub fn build(req: &AssertionBuildRequest) -> Result<Credential> {
    let _data = fetch_notion_data()?;
	match Credential::new(&req.who, &req.shard) {
		Ok(mut credential_unsigned) => {
            credential_unsigned.add_subject_info(VC_DEV_UNDERGRADUATE_SUBJECT_DESCRIPTION, VC_DEV_UNDERGRADUATE_SUBJECT_TYPE);
