...
/// Starts a `ServiceBuilder` for a full service without evm.
///
/// Use this macro if you don't actually need the full service, but just the builder in order to
/// be able to perform chain operations.
pub fn new_partial_without_evm<RuntimeApi, BIQ>(
	config: &Configuration,
	is_standalone: bool,
	build_import_queue_without_evm: BIQ,
) -> Result<
	PartialComponents<
	...
	>,
{
	...
	let import_queue = build_import_queue_without_evm(
		client.clone(),
		block_import.clone(),
		config,
...
pub fn build_import_queue_without_evm<RuntimeApi>(
...
pub async fn start_node_without_evm<RuntimeApi>(
...
where
	...
	sc_client_api::StateBackendFor<ParachainBackend, Block>: sp_api::StateBackend<BlakeTwo256>,
{
	start_node_impl_without_evm::<RuntimeApi, _, _, _>(
		parachain_config,
		polkadot_config,
		collator_options,
		id,
		|_| Ok(RpcModule::new(())),
		build_import_queue_without_evm::<RuntimeApi>,
		|client,
		 block_import,
		 prometheus_registry,
		 telemetry,
	...
	) -> Result<Box<dyn ParachainConsensus<Block>>, sc_service::Error>,
{
	...
	let rpc_builder = {
		...
		Box::new(move |deny_unsafe, _| {
			let deps = rpc_without_evm::FullDepsWithoutEVM {
				client: client.clone(),
				pool: transaction_pool.clone(),
				deny_unsafe,
			};

			crate::rpc_without_evm::create_full_without_evm(deps).map_err(Into::into)
		})
	};
