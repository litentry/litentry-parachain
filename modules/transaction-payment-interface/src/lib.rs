...
/// Represent transaction fee distribution linear proportion
...
pub mod pallet {
	...
	pub struct GenesisConfig<T: Config> {
		pub ratio: RatioOf,
		// pub treasury_proportion: u32,
		// pub author_proportion: u32,
		// pub burned_proportion: u32,
		pub fix_block_reward: u32,
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_fix_block_reward(
			...
			if BalanceOf::<T>::from(block_reward) <
...
mod tests {
	...
	fn signed_extension_transaction_payment_work() {
		ExtBuilder::default()
			...
			.build()
			.execute_with(|| {
				assert_ok!(TransactionPaymentInterface::set_ratio(Origin::root(), RatioOf {treasury: 50, author: 30, burned: 20}));
				...
				assert_ok!(TransactionPaymentInterface::set_ratio(Origin::root(), RatioOf {treasury: 20, author: 30, burned: 50}));
				let mut sender_balance = Balances::free_balance(1);
				let mut treasury_balance = Balances::free_balance(Treasury::account_id());
				let len = 10;
				let pre = pallet_transaction_payment::ChargeTransactionPayment::<Runtime>::from(0)
