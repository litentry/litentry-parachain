...
use sp_std::prelude::*;
use tee_primitives::{
	EthereumSignature, Identity, IdentityHandle, IdentityMultiSignature, IdentityString,
	IdentityWebType, ValidationData, Web2Network, Web3CommonValidationData, Web3Network,
	Web3ValidationData,
};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn verify_identity(
			...
			// do an actual verification for web3
	...
	impl<T: Config> Pallet<T> {
		...
		fn verify_substrate_signature(
			...
			let msg = get_expected_web3_message(who, identity)?;
			Ok(())
		}
		...
		fn verify_evm_signature(
			...
			let hash =
				Self::get_evm_msg_hash(msg).map_err(|_| Error::<T>::UnexpectedEthMsgLength)?;
			Ok(())
		}
		...
		pub fn get_evm_msg_hash(mut msg: Vec<u8>) -> Result<[u8; 32], &'static str> {
			ensure!(msg.len() == 51, Err(format!("wrong msg length {}", msg.len)));
