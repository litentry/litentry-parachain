...
fn join_candidates_event_emits_correctly() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		...
		assert_ok!(ParachainStaking::add_candidates_whitelist(Origin::root(), 1));
...
fn cannot_join_candidates_if_delegator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(Origin::root(), 2));
			assert_noop!(
...
fn insufficient_join_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(Origin::root(), 6));
		});
}
...
fn can_still_delegate_if_leaving() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(Origin::signed(2), 3, 10),);
		});
}
...
fn execute_leave_delegators_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_leave_delegators(Origin::signed(2)));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_delegators(Origin::signed(2), 2));
...
fn execute_leave_delegators_removes_delegations_from_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_delegators(Origin::signed(1), 1));
			for i in 2..6 {
				let candidate_state =
					ParachainStaking::candidate_info(i).expect("initialized in ext builder");
...
fn sufficient_execute_leave_delegators_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			for i in 3..7 {
				assert_ok!(ParachainStaking::execute_leave_delegators(Origin::signed(i), i));
			}
		});
}
...
fn parachain_bond_inflation_reserve_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(25);
			assert_ok!(ParachainStaking::execute_leave_delegators(Origin::signed(6), 6));
			roll_to(30);
			let mut new2 = vec![
...
fn paid_collator_commission_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_events!(expected.clone());
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(Origin::root(), 4));
...
fn remove_white_list() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::remove_candidates_whitelist(Origin::root(), 1));
			...
			assert_ok!(ParachainStaking::remove_candidates_whitelist(Origin::root(), 3));
			assert_last_event!(MetaEvent::ParachainStaking(Event::CandidateWhiteListRemoved {
				candidate: 1,
			}));
		});
...
fn staked_capacity() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// balance
			assert_noop!(
				ParachainStaking::candidate_bond_more(Origin::signed(1), 3000),
				DispatchError::Module(ModuleError {
					index: 2,
					error: [8, 0, 0, 0],
					message: Some("InsufficientBalance")
			...
			assert_noop!(
				ParachainStaking::candidate_bond_more(Origin::signed(3), 500),
				DispatchError::Module(ModuleError {
					index: 2,
					error: [8, 0, 0, 0],
					message: Some("InsufficientBalance")
			...
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(5), 1, 5));
			let state = ParachainStaking::delegation_scheduled_requests(&1);
			assert_eq!(
				state,
				vec![ScheduledRequest {
			...
			assert_ok!(ParachainStaking::schedule_candidate_bond_less(Origin::signed(2), 5));
			roll_to(20);
			assert_ok!(ParachainStaking::execute_candidate_bond_less(Origin::signed(2), 2));
			assert_last_event!(MetaEvent::ParachainStaking(Event::CandidateBondedLess {
				candidate: 2,
				amount: 5,
				new_bond: 595,
