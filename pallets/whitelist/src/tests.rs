...
fn whitelist_on_off_function() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(EnsureWhitelist::<Test>::ensure_origin(RuntimeOrigin::signed(ACCOUNT_B)));
		...
		// Can not pass now
		assert!(EnsureWhitelist::<Test>::ensure_origin(RuntimeOrigin::signed(ACCOUNT_B)).is_err());
	})
}
