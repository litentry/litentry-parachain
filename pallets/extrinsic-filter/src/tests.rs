...
fn set_mode_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(Event::ExtrinsicFilter(crate::Event::ModeSet {
...
fn set_mode_fails_with_unauthorized_origin() {
	new_test_ext().execute_with(|| {
		assert_eq!(ExtrinsicFilter::mode(), crate::OperationalMode::Normal);
		assert_noop!(
			ExtrinsicFilter::set_mode(Origin::signed(1), crate::OperationalMode::Test),
...
fn safe_mode_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(call.dispatch(Origin::none()), frame_system::Error::<Test>::CallFiltered);

		// SafeModeFilter disallows balance calls
		let call: Call = pallet_balances::Call::transfer { dest: 2, value: 10 }.into();
...
fn block_single_extrinsic_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(Event::ExtrinsicFilter(crate::Event::ExtrinsicsBlocked {
		...
		let call: Call = pallet_balances::Call::transfer_keep_alive { dest: 2, value: 10 }.into();
		assert_ok!(call.dispatch(Origin::signed(1)));
		assert_eq!(Balances::free_balance(&2), 10);
	});
}
...
fn unblock_single_extrinsic_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(Event::ExtrinsicFilter(crate::Event::ExtrinsicsUnblocked {
			pallet_name_bytes: b"Balances".to_vec(),
			function_name_bytes: Some(b"transfer".to_vec()),
		}));
		assert_eq!(
...
fn unblock_whole_pallet_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ExtrinsicFilter::unblock_extrinsics(Origin::root(), b"Balances".to_vec(), None));
		System::assert_last_event(Event::ExtrinsicFilter(crate::Event::ExtrinsicsUnblocked {
			pallet_name_bytes: b"Balances".to_vec(),
			function_name_bytes: None,
		}));
...
fn unpaired_block_unblock_fails() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			ExtrinsicFilter::unblock_extrinsics(Origin::root(), b"Balances".to_vec(), None),
			Error::<Test>::ExtrinsicNotBlocked
		);
