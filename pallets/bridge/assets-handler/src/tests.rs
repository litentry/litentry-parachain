...
fn constant_equality() {
	let r_id = pallet_chain_bridge::derive_resource_id(
		1,
		&pallet_chain_bridge::hashing::blake2_128(b"LIT"),
	);
	let encoded: [u8; 32] =
...
#[test]
fn transfer() {
	let dest_bridge_id: pallet_chain_bridge::BridgeChainId = 0;
	...
	new_test_ext_initialized(dest_bridge_id, resource_id, native_token_asset_info).execute_with(
		|| {
			...
			assert_events(vec![
				...
				RuntimeEvent::Bridge(pallet_chain_bridge::Event::FungibleTransfer(
					dest_bridge_id,
					1,
					resource_id,
					100 - 10,
...
fn transfer_to_regular_account() {
	new_test_ext().execute_with(|| {
		let dest_chain = 0;
		let asset = pallet_chain_bridge::derive_resource_id(
			dest_chain,
			&pallet_chain_bridge::hashing::blake2_128(b"an asset"),
		);
		let amount: u64 = 100;
...
fn create_successful_transfer_proposal() {
	let src_id: pallet_chain_bridge::BridgeChainId = 0;
	...
	new_test_ext_initialized(src_id, r_id, native_token_asset_info).execute_with(|| {
		...
		let expected = pallet_chain_bridge::ProposalVotes {
			votes_for: vec![RELAYER_A],
			votes_against: vec![],
			status: pallet_chain_bridge::ProposalStatus::Initiated,
		...
		let expected = pallet_chain_bridge::ProposalVotes {
			...
			votes_against: vec![RELAYER_B],
			status: pallet_chain_bridge::ProposalStatus::Approved,
			expiry: ProposalLifetime::get() + 1,
		};
		assert_eq!(prop, expected);
		...
		assert_events(vec![
			RuntimeEvent::Bridge(pallet_chain_bridge::Event::VoteFor(src_id, prop_id, RELAYER_A)),
			RuntimeEvent::Bridge(pallet_chain_bridge::Event::VoteAgainst(
				src_id, prop_id, RELAYER_B,
			)),
			RuntimeEvent::Bridge(pallet_chain_bridge::Event::VoteFor(src_id, prop_id, RELAYER_C)),
			RuntimeEvent::Bridge(pallet_chain_bridge::Event::ProposalApproved(src_id, prop_id)),
			RuntimeEvent::AssetsHandler(Event::TokenBridgeIn {
				asset_id: None,
				to: RELAYER_A,
				amount: 10,
			...
			RuntimeEvent::Bridge(pallet_chain_bridge::Event::ProposalSucceeded(src_id, prop_id)),
		]);
	})
}
...
fn test_external_balances_adjusted() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(pallet_chain_bridge::Pallet::<Test>::whitelist_chain(
			RuntimeOrigin::root(),
			dest_chain
		));
