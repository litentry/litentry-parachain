...
fn set_admin_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Drop3::set_admin(Origin::signed(1), 2));
		assert_eq!(Drop3::admin().unwrap(), 2);
		System::assert_last_event(Event::Drop3(crate::Event::AdminChanged { old_admin: Some(1) }));
	});
}
...
fn set_admin_fails_with_unprivileged_origin() {
	new_test_ext().execute_with(|| {
		assert_eq!(Drop3::admin().unwrap(), 1);
		assert_noop!(Drop3::set_admin(Origin::signed(2), 2), sp_runtime::DispatchError::BadOrigin);
		assert_eq!(Drop3::admin().unwrap(), 1);
	});
}
...
fn propose_reward_pool_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(Event::Drop3(crate::Event::RewardPoolProposed {
			id: 1,
			name: b"test".to_vec(),
			owner: 3,
		}));
...
fn reject_reward_pool_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_has_event(Event::Drop3(crate::Event::RewardPoolRejected { id: 1 }));
		System::assert_has_event(Event::Drop3(crate::Event::BalanceSlashed { who: 3, amount: 20 }));
		System::assert_has_event(Event::Drop3(crate::Event::RewardPoolRemoved {
			id: 1,
			name: b"test".to_vec(),
			owner: 3,
		}));
...
fn reject_reward_pool_works_with_unexpected_unreserve() {
	new_test_ext().execute_with(|| {
		...
		System::assert_has_event(Event::Drop3(crate::Event::RewardPoolRejected { id: 1 }));
		// only 10 was slashed
		System::assert_has_event(Event::Drop3(crate::Event::BalanceSlashed { who: 3, amount: 10 }));
		System::assert_has_event(Event::Drop3(crate::Event::RewardPoolRemoved {
			id: 1,
			name: b"test".to_vec(),
			owner: 3,
...
fn close_reward_pool_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Drop3::close_reward_pool(Origin::signed(3), 1),
			Error::<Test>::NoSuchRewardPool
		);
	});
}
...
fn send_reward_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Drop3::send_reward(Origin::signed(3), 1, 4, 10));
		assert_ok!(Drop3::send_reward(Origin::signed(3), 1, 5, 20));
...
fn send_reward_fails_with_unapproved_pool() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Drop3::propose_reward_pool(Origin::signed(3), b"test".to_vec(), 100, 1, 3));
		assert_noop!(
			Drop3::send_reward(Origin::signed(3), 1, 4, 10),
...
fn send_reward_fails_with_insufficient_reserved_balance() {
	new_test_ext().execute_with(|| {
		...
		<Balances as ReservableCurrency<_>>::unreserve(&3, 90);
		assert_noop!(
			Drop3::send_reward(Origin::signed(3), 1, 4, 20),
...
fn send_reward_fails_with_insufficient_remain() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Drop3::send_reward(Origin::signed(3), 1, 4, 120),
