...
itp_attestation_handler=debug,http_req=debug,itc_rest_client=debug,\
itp_top_pool=debug,itc_parentchain_indirect_calls_executor=debug"
...
for ((i = 0; i < ${WORKER_NUM}; i++)); do
	...
	for Item in 'enclave.signed.so' 'key.txt' 'spid.txt' 'litentry-worker' 'bitacross-cli'; do
